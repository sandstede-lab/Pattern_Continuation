function [p_history,counts,L_history,p_history_all,metric_history_all] = continuation(contpar, featpar, modelpar, feature_handle, obj_handle, start )
...
while L_current < contpar.L
    ...
    [p_current,counts(i),p_history_new, metric_history_new] = bisect_interval(modelpar, featpar, feature_handle, obj_handle, p_c1, p_c2, dn, max_count);
...
%     
    ...
    if max( metric_history_new ) < contpar.stopping
...
%         [~,idx] = sort([mean(dists_c1(:)),mean(dists_c2(:))]);
        ...
        % end
        if ~strcmp(featpar.feature,'steady') && ~strcmp(featpar.feature,'retract') % avoid homogeneous unless we want that
            if feature_bp_all == 0
                val_c1 = -999;
            end
            if feature_bm_all == 0
