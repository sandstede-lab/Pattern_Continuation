      ...
      */
{
  ...
  if((fp=fopen(filename,"r"))!=NULL) { /* if file can be opened then read it */
    next_line(fp);
    while( (dummy=getc(fp)) != '='); fscanf (fp, "%d, %d", &M1_ic, &M2_ic); 
    next_line(fp); 
    next_line(fp);
    next_line(fp);
    next_line(fp);
    if(verbose) printf("Reading ic.dat with M1 = %d M2 = %d ... \n\n", M1_ic, M2_ic);

    /* Allocate temporary memory (appropriate for in M2_ic) */
    u_tmp =((precision **) malloc((unsigned)(M1_ic)*sizeof(precision*)))-1;
    v_tmp =((precision **) malloc((unsigned)(M1_ic)*sizeof(precision*)))-1;
    for(i=1;i<=M1_ic;i++){
      u_tmp[i]=((precision *) malloc((unsigned)(M2_ic)*sizeof(precision)))-1;
      v_tmp[i]=((precision *) malloc((unsigned)(M2_ic)*sizeof(precision)))-1;
    }
...
#if POLAR
    fscanf (fp, "%lg %lg\n", &u_in, &v_in); /* read central point */
    u[0][1] = u_in; v[0][1] = v_in;
#endif
    
    /* Copy into u and v */
    ratio1 = (M1_ic-1.0)/(M1-1.0);
    ratio2 = (M2_ic-1.0)/(M2-1.0);
    for(i=1;i<=M1;i++) {
      i_tmp = 1+(int)(ratio1*(i-1));
      for(j=1;j<=M2;j++) {
	j_tmp = 1+(int)(ratio2*(j-1));
    ...
    for(i=M1_ic;i>=1;i--) free((char*)(u_tmp[i]+1));
    for(i=M1_ic;i>=1;i--) free((char*)(v_tmp[i]+1));
    free((char*) (u_tmp+1));
    free((char*) (v_tmp+1));
  }
  else {     
	...
	u[0][1] = 1; v[0][1] = a_param/2;  /* central point */
#else
