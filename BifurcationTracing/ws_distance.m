function wsd = ws_distance(u_samples, v_samples, p, norm)
...
if norm
        max_dist = max( [ abs(max(u_samples) - max(v_samples)),abs(max(u_samples) - min(v_samples))]);
        max_dist = max([ max_dist,  abs(min(u_samples) - max(v_samples)),abs(min(u_samples) - min(v_samples))]);
